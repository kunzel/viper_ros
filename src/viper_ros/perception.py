...
#                 "/soma/insert_objects", SOMAInsertObjs
#             )
...
class PerceptionReal (smach.State):
	...
	self.process_scene_srv_name = "/surface_based_object_learning/process_scene"
	#self.pu_srv_name = "/update_person_model"

	rospy.loginfo('Waiting for service %s', self.process_scene_srv_name)
	rospy.wait_for_service(self.process_scene_srv_name)

        try:
           self.ir_service = rospy.ServiceProxy(self.ir_service_name, recognize)
           self.process_scene_srv = rospy.ServiceProxy(self.process_scene_srv_name, ProcessScene)
          # self.person_update_service = rospy.ServiceProxy(self.pu_srv_name, PersonUpdate)
        except rospy.ServiceException, e:
            rospy.logerr("Service call failed: %s" % e)
